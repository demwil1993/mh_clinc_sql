...
SELECT p.id, p.fullname, i.ins_name , i.co_pay , i.percent_covered 
FROM patient p 
...
SELECT p.id, p.fullname, date_part('year', age(p.dob)) AS age, i.ins_name ,
...
------------------------------ patients --------------------------------
/*find patients who have had an assesment and therapy SESSION. output patient name, 
 * therapist name, psychologist name, type of therapy, 
 * type of assessment
 */
SELECT p.fullname AS pat_name, t.fullname AS the_name, t2.therapy_type , 
	cs.session_date, p2.fullname AS psy_name, a.testname AS assest_type , h.app_date 
FROM patient p 
